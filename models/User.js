        ...
        if (_.has(input, 'profile') || _.has(input, 'cover')) {
            ...
            return mediaObj.get().then(function (media_array) {
                ...
                _.forEach(_usersRaw, function (value, key) {
                    if (_.has(input, 'profile')) {
                        value.profile = _.head(_.filter(media_array, function (o) {
                            return o.id == value.profile_image_id;
                        }));
                    }
                    if (_.has(input, 'cover')) {
                        value.cover = _.head(_.filter(media_array, function (o) {
                            return o.id == value.cover_image_id;
                        }));
                    }
                });
    ...
    for (var key in input) {
        ...
        if (!input.hasOwnProperty(key)) {
            ...
            if (key == 'location') {
                ...
                query_models.user_defined_location = {
                    model: models.user_defined_location,
                    attributes: ['id', 'formatted_address', 'locality', 'administrative_area_level_1', 'country']
                };
            }
        }
    }
